...
const moreRecipesContainer = document.querySelector(".more-recipes");
...
if (recipe) {
  ...
  const htmlRecipe = `
    ...
    <div class="recipe-card-txt">
      <h1 class="tp-2">${recipe.title}</h1>
      <p class="tp-6">${recipe.overview}</p>
      ...
      <h2 class="tp-4">Ingredients:</h2>
      <ul>
        ${ingredientsList}
      </ul>
      <h2 class="tp-4">Instructions:</h2>
      <ul>
        ${instructionsList}
      </ul>
    </div>
  ...
  document.querySelector(".recipe-card").innerHTML = htmlRecipe;
} else {
  ...
  recommended.forEach((r) => {
    const moreRecipe = `
      <div class="recipe-wrapper">
        <img class="recipe-img-lg" src="../assets/images/${r.image.large}" alt="${r.overview}">
        <img class="recipe-img-small" src="../assets/images/${r.image.small}" alt="${r.overview}">
        <p class="recipe-title"><strong class="tp-5">${r.title}</strong></p>
        <p class="recipe-detail tp-9">${r.overview}</p>
        <div class="recipe-infos">
          <div class="recipe-info">
            ...
            <p class="recipe-info-txt tp-9">Servings: ${r.servings}</p>
          </div>
          <div class="recipe-info recipe-info-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" fill="none" viewBox="0 0 21 20">
              <path fill="#163A34" fill-rule="evenodd"
            ...
            <p class="recipe-info-txt tp-9">Prep: ${r.prepMinutes} mins</p>
          </div>
          <div class="recipe-info">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 20 20">
              <path fill="#163A34"
            ...
            <p class="recipe-info-txt tp-9">Cook: ${r.cookMinutes} mins</p>
          </div>
        </div>
        <button class="recipe-view tp-8" data-id="${r.id}">View Recipe</button>
      </div>
    `;

    moreRecipesContainer.insertAdjacentHTML("beforeend", moreRecipe);
  });

  const viewBtns = document.querySelectorAll(".more-recipes .recipe-view");
  viewBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      const allRecipes = JSON.parse(localStorage.getItem("RECOMMENDED"));
